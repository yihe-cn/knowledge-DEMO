// design.jsx — shared neumorphic primitives & theme

const { useState, useEffect, useRef, useMemo, useCallback } = React;

// ─── Theme ─────────────────────────────────────────────────────
// Two palettes × dark/light. Neumorphism = soft tonal base with paired light/dark shadows.
const THEMES = {
  cream: {
    bg: '#ECE7DD',
    surface: '#ECE7DD',
    ...
    accent: '#2C5F5A',     // deep teal
    accentSoft: '#B8743A', // copper
    good: '#3D8B5C',
    warn: '#C97A3D',
    bad: '#C25A4F',
  },
...
// ─── Neumorphic surfaces ────────────────────────────────────────
// raised: outer shadows. inset: inner shadows. flat: gentle separator.
function neuRaised(t, r = 18, depth = 1) {
...
function neuInset(t, r = 18, depth = 1) {
...
function Card({ children, t, style = {}, inset = false, radius = 22, depth = 1, onClick }) {
...
function PillButton({ children, t, onClick, primary = false, disabled = false, style = {} }) {
  return (
    <button
...
// ─── Status / nav bar ──────────────────────────────────────────
function PhoneStatusBar({ t, dark }) {
...
function TopBar({ t, title, onBack, right }) {
  return (
...
Object.assign(window, {
  THEMES, useTheme,
  neuRaised, neuInset, neuFlat,
  Card, PillButton, Icon,
  PhoneStatusBar, TopBar,
});
